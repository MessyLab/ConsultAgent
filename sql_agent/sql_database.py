...
from sqlalchemy import MetaData, create_engine, insert, inspect, text, select
...
class SQLDatabase:
    ...
    def get_table_indexes(self, table_name: str) -> str:
        """Get the create command of table"""
        table = self._metadata.tables[table_name]
        create_table_command = str(CreateTable(table))
        # create_table_command_formatted = "\n".join(create_table_command)
    ...
    def get_sample_rows(self, table_name: str) -> str:
        """Get the sample rows from table"""
        ...
        table = self._metadata.tables[table_name]
        query = select(table).limit(self._sample_rows_in_table_info)
